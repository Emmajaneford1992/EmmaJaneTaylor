...
export default function WobblyBlob({ ...props }) {
  ...
  const [{ wobble, coat, color, ambient, env, opacity }] = useSpring(
    {
      wobble: down ? 1 : hovered ? 1 : 1,
    ...
    [mode, hovered, down, props.color, props.opacity]
  )
      ...
      <Suspense >
        <a.mesh
         ...
         // onPointerOut={() => setHovered(false)}
          ...
          <AnimatedMaterial color={0xffffff} envMapIntensity={env} clearcoat={coat} clearcoatRoughness={0.1} metalness={0.1} opacity={opacity} transparent={true} depthWrite={false} depthTest={false} />
        </a.mesh>
